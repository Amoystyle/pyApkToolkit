...
class ShellThread(QtCore.QThread):
    signal_shell = QtCore.Signal(int, str)  # 决定信号接受的参数类型
...
class MyWindow(QtWidgets.QMainWindow, Ui_Form):
    ...
    def on_pushButton_CheckEvn(self):
        ...
        if output.find("verify") >= 0:
            ...
            self.addLog(
                "jarsigner.exe：无法检测到，请在环境变量中设置JAVA_HOME和PATH，例如：\r\n"
    ...
    def on_pushButton_SignApk(self):
        ...
        if os.path.isfile(path):
            ...
            self.shellThread(
                "SignApk",
                'java -jar "{}" sign --ks "{}" --ks-pass pass:{} --ks-key-alias {} --key-pass pass:{} \
                --v1-signing-enabled {} --v2-signing-enabled {} --in "{}" --out "{}"'.format(
                    self.binToolPath + self.apksigner, keyStorePath, storePassword, keyAlias, keyPassword, str(v1).lower(), str(v2).lower(), path, outPath
                ),
            )
